...
\adjustbox{valign=t}{\begin{minipage}{13.1cm}
    ...
    {\icon{Python} Python,\icon{pytest} pytest,\icon{flask} Flask,\icon{HTML5} HTML,\icon{Bootstrap} Bootstrap,\icon{CSS3} CSS,\icon{Sqlite} SQLite,\icon{Git} git,{\includegraphics[align=c, height=9.5pt]{PIC/Ubuntu.png}} Ubuntu,\icon{latex.png} \icon{LaTeX}}
...
\adjustbox{valign=t}{\begin{minipage}{20.4cm}
    ...
    \noindent
    \section*{\colorbox{sideBarColor!40}{\begin{minipage}{0.99\textwidth}
    \centering{\textbf{{\faCertificate} 專案 \vphantom{p\^{E}}}}
    \end{minipage}}}
    ...
    {\faHashtag\space Overleaf,\faHashtag\space Pycharm,\faHashtag\space VS code,\faHashtag\space Anaconda,\faHashtag\space Ollama}
...
我們應用程式的特色是關於持續監控和協助解決不斷擴大的老年人口可能造成的問題。\\
    ...
    {\faHashtag\space 預防性維護,\faHashtag\space 矯正性維護,\faHashtag\space 例行性維護}
